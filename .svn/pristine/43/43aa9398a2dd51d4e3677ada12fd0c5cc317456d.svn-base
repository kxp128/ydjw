$(function () {
    $("#jqGrid").jqGrid({
		...
		postData:{currentCensusType: 5},
        datatype: "json",
        colModel: [
            { label: '户籍类型', name: function (obj) {
                if(obj.currentCensusType==4){
                    return "夫妻投靠迁入"
                }else if(obj.currentCensusType==5){
                    return "新生儿入户"
                }else if(obj.currentCensusType==6){
                    return "户籍注销"
                }
            }, index: 'name', width: 80 },
			...
			{ label: '新生儿姓名', name: 'newbornName', index: 'newborn_name', width: 80 },
            { label: '新生儿性别', name: 'newbornGender', index: 'newborn_gender', width: 80 },
			{ label: '新生儿出生时间', name: function (obj) {
				return obj.newbornBirthDate==null?null:obj.newbornBirthDate.substring(0,11);
            }, index: 'married_time', width: 80 },
